...
import { deleteUser, fetchUsers, UserOut } from "@/utils/api";
import {
  getCoreRowModel,
  getFilteredRowModel,
...
export default function UserTable() {
  ...
  const handleDelete = async (userId: number) => {
    try {
      ...
      setUser((prev) => prev.filter((p) => p.id !== userId));
    } catch (err) {
      console.error(err);
    }
  };
  const handleEdit = async (user: UserOut) => {
    router.push(`user/edit-user?id=${user.id}`);
  };
